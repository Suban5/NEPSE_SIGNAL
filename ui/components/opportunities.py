"""Opportunities panel rendering for Streamlit UI."""
...
ANALYTICS_ENDPOINT = "/analytics/opportunities"
...
def render(client: ApiClient, api_version: str) -> None:
    ...
    with col_top_n:
        top_n = int(st.number_input("Top N", min_value=1, max_value=200, value=20, step=1, key="opps_top_n"))
    with col_sector:
        sector_relative = bool(st.checkbox("Sector Relative", value=False, key="opps_sector_relative"))
    with col_refresh:
        fetch_clicked = st.button("Fetch Opportunities", key="opps_fetch")

    if not fetch_clicked:
        render_fetch_hint("Click 'Fetch Opportunities' to load data from /analytics/opportunities")
        return

    with st.spinner("Fetching opportunities..."):
        try:
            response = client.analytics_opportunities(
                top_n=top_n,
    ...
    if not isinstance(rows, list) or not rows:
        render_empty_state("opportunities")
        return
