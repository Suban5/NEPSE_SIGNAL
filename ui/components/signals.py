"""Signals panel rendering for Streamlit UI."""
...
ANALYTICS_ENDPOINT = "/analytics/signal-summary"


def render(client: ApiClient, api_version: str) -> None:
    """Render the Top Trade Signals panel."""
    st.subheader("Top Trade Signals")

    col_top_n, col_sector, col_refresh = st.columns(3)
    with col_top_n:
        top_n = int(st.number_input("Top N", min_value=1, max_value=200, value=20, step=1, key="signals_top_n"))
    with col_sector:
        sector_relative = bool(st.checkbox("Sector Relative", value=False, key="signals_sector_relative"))
    with col_refresh:
        fetch_clicked = st.button("Fetch Signals", key="signals_fetch")

    if not fetch_clicked:
        render_fetch_hint("Click 'Fetch Signals' to load data from /analytics/signal-summary")
        return

    with st.spinner("Fetching signal summary..."):
        try:
            response = client.analytics_signal_summary(
                top_n=top_n,
    ...
    if not isinstance(rows, list) or not rows:
        render_empty_state("signal-summary")
        return
